...
protected:
    ...
    StrictMock<MockSimpleShellCallbacks> mockCallbacks;
    void* testUserData;
...
TEST_F(WesterosSimpleShellTest, NotifySurfaceDestroyedWithValidParameters) {
    ...
    // Expect get_name callback to be called during destruction
...
TEST_F(WesterosSimpleShellTest, InterfaceMethod_SetZorder_CallsCallback) {
    ...
    // Expect set_zorder callback (zorder 10.0)
    EXPECT_CALL(mockCallbacks, set_zorder(testUserData, 222, 10.0f)).Times(1);
...
TEST_F(WesterosSimpleShellTest, VerifyInterfaceStructure_ContainsCorrectFunctions) {
    // Act & Assert - Verify interface function pointers
    ASSERT_EQ(simple_shell_interface.set_name, wstSimpleShellSetName);
    ASSERT_EQ(simple_shell_interface.set_visible, wstSimpleShellSetVisible);
    ASSERT_EQ(simple_shell_interface.set_geometry, wstSimpleShellSetGeometry);
    ASSERT_EQ(simple_shell_interface.set_opacity, wstSimpleShellSetOpacity);
    ASSERT_EQ(simple_shell_interface.set_zorder, wstSimpleShellSetZorder);
    ASSERT_EQ(simple_shell_interface.get_status, wstSimpleShellGetStatus);
    ASSERT_EQ(simple_shell_interface.get_surfaces, wstSimpleShellGetSurfaces);
    ASSERT_EQ(simple_shell_interface.set_focus, wstSimpleShellSetFocus);
    ASSERT_EQ(simple_shell_interface.set_scale, wstSimpleShellSetScale);
}
